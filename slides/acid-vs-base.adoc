...
t2 --- read y <- 3 --- write x -> 7 --- commit
```

* at start `x = 2`, `y = 3`
* constraint defined: `a + b =< 10`

[NOTE.speaker]
--
perfect summary at: https://blog.acolyer.org/2016/02/24/a-critique-of-ansi-sql-isolation-levels
--
...
Atul Adya: isolation levels

image:{images}/cap/adya-transaction-level-ordering.png[role="noborder"]
...
When mutable state is distributed over multiple machines each machine can receive update events
...
A distributed system is consistent exactly when the outside world can never observe two different serialisations.
...
Causal consistency (https://www.quora.com/Distributed-Systems-What-is-a-causal-consistency)

* A system is causally consistent if all agents see (only) causally related events in the same order.
  This is weaker than sequential consistency because the later requires all events appear in the same order,
  irrespective of causality. My favorite example is from http://www-bcf.usc.edu/~wyattllo/papers/causal-login13.pdf
* Causal consistency improves user experience because with it actions appear to everyone in the correct order.
  A common scenario where causality is important, but often isn’t provided,
  is comments on social network posts, which sometimes appear out of order.
**  e.g.
...
* it prefers consistency over availability
* if network partion occurs then part with majority can work,
  part with minority is stuck
** expects some consensus protocol being run in the underlaying
...
* https://raft.github.io/raft.pdf (paper on Raft)
* https://www.youtube.com/watch?v=JEpsBg0AO6o (Paxos lecture (Raft user study), John Ousterhout)
--
...
* https://www.youtube.com/watch?v=0UTOLRTwOX0 (Distributed Sagas: A Protocol for Coordinating Microservices, Caitie McCaffrey, JOTB17)
--
...
[NOTE.speaker]
--
Eventual consistency with strong guarantees : CALM, ACID 2.0, CRDT
...
What the documentation on Bloom language says

```
One of the key innovations underlying Bloom is the ability to formally guarantee consistency
...
but Bloom’s roots in logic make it easy for us to convert the theory into practical tools for Bloom programmers.
```

In some simplistic way we can say that CALM says that the system can grow only in one
direction. As counter that can only increase its value and no going down.

I-confluence by Peter Bailis, by his words

```
CALM says that, if your program logic is monotonic http://bloom-lang.net/calm/,
...
user-specified invariant, you can preserve that invariant without coordination.
```

ACID 2.0

Reusing ACID for distributed systems.

When all operation in distributed systems bind the properties of ACID 2.0 then
we get the CRDT. I understand it as CRDT is "the implementation" of the ACID 2.0
definition.

Example as everytime is balance of an account. We can say starting balance 50, then
decrease for 10 and later increase for 20, we have 60.

The acid 2.0 is about no mutability permitted. We rather saying starting balance at time and then
...
and associativity we don't care about order that the records are received by nodes.
...
* http://www.enterpriseintegrationpatterns.com/ramblings/68_acid.html
* https://medium.com/@istanbul_techie/a-look-at-conflict-free-replicated-data-types-crdt-221a5f629e7e
--
...
* Spanner uses pesimistic locking
* Spanner upstream "implementation" is CocroachDB

* The point I understood from the paper is that serializibility is ensured by 2PC (and two phase locking),
  lineriazibility is possible because of TrueTime.
* Commit of transaction is delayed to be at the end of delta time window where
  time uncertainty resides. When committed at the end we know there was no other txn interleaving with data
  and lineriazibility is ensured.


* https://research.google.com/pubs/pub45855.html (Spanner, TrueTime and the CAP Theorem)
* https://research.google.com/archive/spanner.html (Spanner: Google's Globally-Distributed Database)
...
* a process is blocked if it must wait for the recovery of another process before deciding
** 2PC is blocking protocol - after prepare there is waiting for TM to recover, there is no way for participant to decide on his own
** 3PC is non-blocking - there is always timeout after which participants can vote (choose) new leader to finish the transaction
** no atomic commit protocol is non blocking if communication failures or total failures, but 2PC can block even with non-total failures
** (https://www.experts-exchange.com/questions/24673620/why-is-3-phase-commit-protocol-non-blocking.html)
*** 3PC: "can you commit", "prepare to commit" "do commit"
*** 2PC: "ready to commit", "commit if you can (the first part "ready to commit" must block the resources until it is either committed or failed, hence blocking)
*** The main reason is 3-phrase commits have a built in timeout.  So any resources waited on are released if the transaction fails to complete (commit or rollback) within the threshold.
    As opposed to 2-phase which can/will hold resources indefinitely.
    So yes, both protocols do block  but 3phrase has a built in release so no blocking is permanent.
** For 2PC, the coordinator may abort the transaction globally or resend the global decision. For a participant, it can leave the process blocked until communication with the coordinator is re-established such as sending abort message to the coordinator or invoke the cooperative termination protocol.
   For 3PC, the coordinator can abort the transaction globally, send global-commit message to the participants or simply send the global decision to all sites that have not acknowledged. The participant can unilaterally abort a transaction, follow an election protocol, or elect a new coordinator.
...
* http://www.cs.utexas.edu/users/lorenzo/corsi/cs380d/past/03F/notes/9-9.pdf + http://www.cs.utexas.edu/users/lorenzo/corsi/cs380d/past/03F/notes/9-11.pdf
 ...
 * http://dl.acm.org/citation.cfm?id=1132867
--
...
[NOTE.speaker]
--
For Perclator and RAMP - we talk about transactions which is capable to work multiple records and provide
...
* https://www.youtube.com/watch?v=YPbGW3Fnmbc (DevoxxUK 17, Using sagas to maintain data consistency in a microservice architecture, Chris Richardson)
