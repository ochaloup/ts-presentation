...
[NOTE.speaker]
--
 In many sources under term distributed transaction is meant XA. We (from Narayna/JBoss/EAP point of view) understands
 distributed transactions such ones that pass its context over a wire/to a different TM.
 XA transaction is such that contains several resources, such specified by X/Open XA architecture - meaning transaction 2PC protocol implemented
 !! This talk won't be about neither XA or distributed transactions (distributed maybe in sense of context passing between EJB beans)
--
...
== JTA and underlying Transaction Manager
 ...
 * 1.0 - UserTransaction and all the stuff
