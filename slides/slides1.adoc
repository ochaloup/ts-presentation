...
--
 ...
 Does @Inject support injecting transaction?
 MDB and BMT
...
--
 ...
 TODO: The container must throw the java.lang.IllegalStateException if an instance of a bean with BMT attempts to invoke the setRollbackOnly() or getRollbackOnly() method of the EJBContext interface.
--
...
--
 ...
 TODO: This will throw an IllegalStateException
--
...
== Exception handling
 ...
 In difference from CMT you have to handled it on your own.

[NOTE.speaker]
--
 TODO: What happens when runtimeexception is thrown? What happens when application exception is thrown?
       Is transaction rollbacked?
--
 ...
 * MANDATORY - no txn then TransactionRequiredException or joining the existing txn
 ...
 * SUPPORTS - no txn then nothing is done (method runs without txn) or joining the existing txn
    - container is lazy :)
 * NEVER - no txn then nothing is done (method runs without txn) or TransactionRequiredException if calling method propagates some started txn
...
== Exception handling
 ...
 Exception which causes rollback (setRollbackOnly called) of current transaction
